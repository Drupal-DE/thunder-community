...
class PathProcessorForum implements InboundPathProcessorInterface, OutboundPathProcessorInterface {
  ...
  /**
   * Path pattern: Forum taxonomy term overview.
   */
  const PATH_PATTERN_TAXONOMY_TERM_OVERVIEW = '!/admin/structure/taxonomy/manage/forums/overview$!';
  ...
  public function processOutbound($path, &$options = [], Request $request = NULL, BubbleableMetadata $bubbleable_metadata = NULL) {
    // Rewrite forum-related taxonomy term paths.
    if (($term = $this->rewriteForumTermPath($path)) && $bubbleable_metadata) {
  ...
  protected function rewriteForumTermPath(&$path) {
    ...
    $patterns = [
      ...
      static::PATH_PATTERN_TAXONOMY_TERM_OVERVIEW,
    ];
    ...
    foreach ($patterns as $pattern) {
      ...
      if (preg_match($pattern, $path, $matches)) {
        ...
        elseif ($pattern === static::PATH_PATTERN_TAXONOMY_TERM_OVERVIEW) {
          $path = '/' . ltrim(Url::fromRoute('forum.overview')->getInternalPath(), '/');
        }
